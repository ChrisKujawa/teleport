...
func (r *respBodyReadCloser) Read(p []byte) (int, error) {
	...
	respRecieved, err := mcputils.UnmarshalResponse(msg)
	if err != nil {
...
func (t *sessionTransport) RoundTrip(r *http.Request) (*http.Response, error) {
	externalSessionID := r.Header.Get("Mcp-Session-Id")
	if externalSessionID != "" {
		t.sessionCtx.mcpSessionID.Store(&externalSessionID)
		t.sessionID = session.ID(uuid.NewSHA1(uuid.Nil, []byte(externalSessionID)).String())
	}
	...
	case len(reqBody) > 0:
		baseMessage, err := mcputils.UnmarshalMessage(string(reqBody))
		if err != nil {
			return nil, trace.Wrap(err)
		}
		switch {
	...
	case "application/json":
		...
		respFromServer, err := mcputils.UnmarshalResponse(string(respBody))
		if err != nil {
			return nil, trace.Wrap(err)
		}
